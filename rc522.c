...
{ 
	...
	Write_MFRC522(TModeReg,0x8D);				//TAuto=1?????????????????§¿??????4¦Ë?????????4¦Ë
	//Write_MFRC522(TModeReg,0x1D);				//TAutoRestart=1????????????0x0D3E??0.5ms???????//test    
	Write_MFRC522(TPrescalerReg,0x3E); 	//????????8¦Ë     
	Write_MFRC522(TReloadRegL,0x32);		//?????????8¦Ë                
	Write_MFRC522(TReloadRegH,0x00);		//?????????8¦Ë       
...
u8 MFRC522_ToCard(u8 command, u8 *sendData, u8 sendLen, u8 *backData, u16 *backLen) 
{
	...
	//
	Write_MFRC522(ComIEnReg, irqEn|0x80);		//?????§Ø?????     
	ClearBitMask(ComIrqReg, 0x80);  				//????????§Ø?????¦Ë               	
...
{     
	...
	for (i=0; i<4; i++)					
	{
		buffer[i+2] = *(serNum+i);	//buffer[2]-buffer[5]??????§Ü?
		buffer[6]  ^=	*(serNum+i);	//??§µ????
	}
	//
	CalulateCRC(buffer, 7, &buffer[7]);	//buffer[7]-buffer[8]?RCR§µ????
